...
interface ProductItemProps {
  product: Product;
  onUpdate: (id: number, action: "increment" | "decrement") => void;
...
const ProductItem: React.FC<ProductItemProps> = ({
  ...
  >
    <button
      ...
      className="absolute top-2 right-2 w-8 h-8 rounded-full bg-white flex justify-center items-center"
    >
    ...
    <Image
      ...
      width={145}
      height={100}
      className="w-full md:w-[145px] h-[100px] object-cover"
    />
    <div className="flex-1 flex flex-col justify-between">
      <div className="font-bold text-lg md:text-xl">{product.product_name}</div>
      <div className="flex items-center gap-2">
        <button
          onClick={() => onUpdate(product.product_id, "decrement")}
        ...
        <button
          onClick={() => onUpdate(product.product_id, "increment")}
          className="bg-white border w-8 h-8 flex items-center justify-center font-bold rounded-full"
        >
          +
        </button>
  ...
  <div className="w-full max-w-sm lg:max-w-[504px] h-auto bg-white shadow-lg rounded-lg overflow-hidden">
    <div className="p-6">
      ...
      <div className="flex flex-col gap-4 mb-4">
        ...
        <div className="flex justify-between">
          <span>Сумма:</span>
          <span className="font-semibold">{totalSum.toLocaleString()} UZS</span>
        </div>
      </div>
      <form>
        <div className="mb-4">
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          ...
          >
            <Image src={click} alt="click" className="w-35 h-10" />
          </button>
          <button
            type="button"
          ...
          <div className="flex flex-col justify-center items-center sm:justify-center gap-6 md:gap-8 lg:flex-row">
            <div className="w-full max-w-md md:max-w-3xl lg:w-2/3 h-auto rounded-lg bg-white p-4 md:p-6">
              ...
              <p className="max-w-md">
                Если у вас имеется вопросы позвоните по номеру:+998 (93) 241 31
                61
              </p>
            </div>
            <OrderSummary totalSum={totalSum} productCount={products.length} />
          </div>
        </div>
      </div>
    </>
